...
import com.example.tttn_electronicsstore_manager_admin_app.api.adminApiService.ImportReceiptService
...
@HiltViewModel
class ImportReceiptViewModel @Inject constructor(private var sharedPref: SharedPreferences) :
    ViewModel() {
    ...
    val _addReceipt = MutableSharedFlow<Resource<ImportReceipt>>()
    val addReceipt = _addReceipt.asSharedFlow()
    ...
    fun addImportReceipt(addImportReceiptRequest: AddImportReceiptRequest) {

        viewModelScope.launch {
            _addReceipt.emit(Resource.Loading())
            val response = importReceiptService.addReceipt(addImportReceiptRequest)
            if (response.isSuccessful) {
                _addReceipt.emit(Resource.Success(response.body()!!.data))
            } else {
                _addReceipt.emit(Resource.Error("Đã xảy ra lỗi"))
            }
        }
    }
